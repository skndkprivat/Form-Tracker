...
// ─── CSV parsere ─────────────────────────────────────────────────────────────
// Returnerer { date: "YYYY-MM-DD", tss: number, name: string }[]
function parseCSV(text) {
  ...
  // ── Intervals.icu fitness CSV (Activities export) ──────────────────────────
  // Kolonner: Date, Name, ..., Training Load (TSS)  ELLER  icu_training_load
  const idxDate = cols.findIndex(c => c === "date" || c === "start_date_local");
  const idxTSS  = cols.findIndex(c =>
    c === "training load" || c === "icu_training_load" ||
    c === "tss" || c === "training stress score"
  );
  ...
  if (idxDate !== -1 && idxTSS !== -1) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const rawDate = cells[idxDate]?.replace(/"/g, "").trim();
      const rawTSS  = cells[idxTSS ]?.replace(/"/g, "").trim();
      const rawName = idxName !== -1 ? cells[idxName]?.replace(/"/g, "").trim() : "";
      const date = normalizeDate(rawDate);
      const tss  = parseFloat(rawTSS);
      if (date && !isNaN(tss)) rows.push({ date, tss: Math.round(tss), name: rawName || "Importeret" });
    }
    return { rows, format: "intervals.icu / Strava CSV" };
  }

  // ── Garmin Connect Activities CSV ──────────────────────────────────────────
  // Kolonner: Activity Type, Date, Favorite, Title, ..., Training Stress Score™
  ...
  if (idxGDate !== -1 && idxGTSS !== -1) {
    ...
    for (let i = 1; i < lines.length; i++) {
      const cells = splitCSVLine(lines[i]);
      const rawDate = cells[idxGDate]?.replace(/"/g, "").trim();
      const rawTSS  = cells[idxGTSS ]?.replace(/"/g, "").trim().replace(",", ".");
      const rawName = idxGName !== -1 ? cells[idxGName]?.replace(/"/g, "").trim() : "";
      const date = normalizeDate(rawDate);
      const tss  = parseFloat(rawTSS);
      if (date && !isNaN(tss) && tss > 0) rows.push({ date, tss: Math.round(tss), name: rawName || "Garmin aktivitet" });
    }
    return { rows, format: "Garmin Connect CSV" };
  }

  // ── Simpel fallback: bare dato + tss kolonner ──────────────────────────────
  ...
  return { rows: [], format: "ukendt (ingen dato+TSS kolonner fundet)" };
}
...
function computeSeries(startCTL, startATL, startDate, planDays, mountains, restDays, logged) {
  ...
  const lastMountainDate = mountains.length
    ? mountains.reduce((max, m) => m.date > max ? m.date : max, mountains[0].date)
    : null;
  ...
  if (lastMountainDate && lastMountainDate > startIso) {
    const diff = Math.round(
      (new Date(lastMountainDate + "T12:00:00") - new Date(startIso + "T12:00:00")) / 86400000
    );
  ...
  return Array.from({ length: totalDays }, (_, i) => {
    ...
    return {
      ...
      planTss, tss,
      logged: !!logEntry, logNote: logEntry?.note || "",
...
function ImportModal({ onImport, onClose }) {
  ...
  const [mergeMode, setMergeMode] = useState("overwrite"); // overwrite | keep
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const { rows, format: fmt } = parseCSV(text);
      if (rows.length === 0) {
        setError(`Kunne ikke læse TSS-data fra filen.\nFormat detekteret: ${fmt}\n\nSe vejledningen nedenfor.`);
        setStep("choose");
      } else {
        setFormat(fmt);
        ...
        {step === "choose" && (
          <>
            <div className="import-header">
              <span className="import-badge">📥 Importer træninger</span>
            </div>
            {error && <div className="import-error"><pre>{error}</pre></div>}

            <div className="import-sources">
              <div className="import-source-card">
                <div className="import-source-title">🟣 Intervals.icu</div>
                <div className="import-source-steps">
                  <div className="step">1. Gå til <strong>intervals.icu</strong> → log ind</div>
                  <div className="step">2. Klik <strong>Aktiviteter</strong> i venstre menu</div>
                  <div className="step">3. Øverst til højre: klik <strong>⋮ → Eksporter CSV</strong></div>
                  <div className="step">4. Gem filen og upload den her ↓</div>
                </div>
              </div>
              <div className="import-source-card">
                <div className="import-source-title">🔵 Garmin Connect</div>
                <div className="import-source-steps">
                  <div className="step">1. Gå til <strong>connect.garmin.com</strong></div>
                  <div className="step">2. Aktiviteter → vælg datointerval</div>
                  <div className="step">3. Klik <strong>Eksporter CSV</strong> (øverst til højre)</div>
                  <div className="step">4. Upload filen her ↓</div>
                </div>
              </div>
              <div className="import-source-card">
                <div className="import-source-title">🟠 Strava</div>
                <div className="import-source-steps">
                  <div className="step">1. Gå til <strong>strava.com</strong> → Indstillinger</div>
                  <div className="step">2. <strong>Mit konto → Download dine data</strong></div>
                  <div className="step">3. Pak ZIP ud og find <strong>activities.csv</strong></div>
                  <div className="step">4. Upload filen her ↓  <em>(TSS kræver Premium)</em></div>
                </div>
              </div>
            </div>

            <div className="import-manual-note">
              💡 Har du ikke en af disse? Du kan også lave en simpel CSV med to kolonner: <code>date,tss</code>
            </div>
        ...
        {step === "preview" && (
          <>
            ...
            <div className="import-stats">
              <strong>{preview.length}</strong> træninger fundet
              {preview.length === 200 && " (viser maks 200)"}
            </div>
            <div className="import-merge">
              <label className="merge-opt">
                <input type="radio" name="merge" value="overwrite" checked={mergeMode==="overwrite"} onChange={()=>setMergeMode("overwrite")} />
                Overskriv eksisterende dage
              </label>
              <label className="merge-opt">
                <input type="radio" name="merge" value="keep" checked={mergeMode==="keep"} onChange={()=>setMergeMode("keep")} />
                Behold eksisterende, spring dublikater over
              </label>
            </div>
            <div className="import-preview-list">
              ...
              {preview.length > 8 && (
                <div className="import-preview-more">... og {preview.length - 8} flere</div>
              )}
            </div>
            <div className="modal-btns">
              <button className="btn-save" onClick={handleImport}>Importer {preview.length} træninger</button>
        ...
        {step === "done" && (
          <>
            <div className="import-done">
              ...
              <div className="import-done-title">{importedCount} træninger importeret</div>
              <div className="import-done-sub">CTL/ATL/TSB er opdateret med dine faktiske data</div>
            </div>
...
function LogModal({ row, onSave, onSkip }) {
  ...
  const handleSave = () => {
    const n = parseFloat(tss.replace(",", "."));
    onSave({ tss: isNaN(n) ? 0 : n, note });
  };
  return (
    <div className="modal-overlay">
      <div className="modal log-modal">
        ...
        <label className="log-field">
          ...
          <input type="text" inputMode="decimal" value={tss}
            onChange={e => setTss(e.target.value)} placeholder={String(row.planTss || 0)} autoFocus />
        </label>
        <label className="log-field">
          <span>Note (valgfri)</span>
          <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="Føltes godt, modvind..." />
        </label>
        <div className="modal-btns">
          <button className="btn-save" onClick={handleSave}>Gem ✓</button>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <label>TSS<input type="text" inputMode="decimal" value={form.tss}
          onChange={e => set("tss", e.target.value)}
          onBlur={e => { const n = parseFloat(String(e.target.value).replace(",",".")); if (!isNaN(n)) set("tss", n); }} /></label>
        <label>Farve<input type="color" value={form.color} onChange={e => set("color", e.target.value)} /></label>
        <div className="modal-btns">
          <button className="btn-save" onClick={() => onSave(form)}>Gem</button>
          <button className="btn-cancel" onClick={onClose}>Annuller</button>
        </div>
      </div>
    </div>
...
function ChartTooltip({ active, payload }) {
  ...
  return (
    <div className="chart-tooltip">
      ...
      {d?.isMountain && <div className="tt-mountain" style={{ color: d.mountainColor }}>🏔️ {d.mountainName}</div>}
...
export default function FormTracker() {
  ...
  const series       = useMemo(() =>
    computeSeries(startCTL, startATL, startDate, DEFAULT_PLAN, mountains, restDays, logged),
    [startCTL, startATL, startDate, mountains, restDays, logged]
  );
  ...
  useEffect(() => {
    ...
    if (row && !row.isMountain) {
      const t = setTimeout(() => setLogModal(row), 600);
  ...
  const saveLog = (date, entry) => {
    setLogged(prev => ({ ...prev, [date]: entry }));
    setLogModal(null);
  ...
  const handleImport = (rows, mode) => {
    setLogged(prev => {
      const next = mode === "keep" ? { ...prev } : { ...prev };
      rows.forEach(r => {
        if (mode === "keep" && next[r.date]) return;
        next[r.date] = { tss: r.tss, note: r.name !== "Importeret" ? r.name : "" };
      });
      return next;
    });
    setSaveMsg(`${rows.length} træninger importeret ✓`);
    setTimeout(() => setSaveMsg(""), 3000);
  };

  const saveMountain = m => {
    setMountains(prev => prev.map(p => p.id === m.id ? { ...m, tss: +m.tss } : p));
    ...
    <div className="app">
      ...
      <header>
        <div className="header-top">
          ...
          <button className="btn-import-header" onClick={() => setShowImport(true)}>
            📥 Importer træninger
          </button>
        </div>
        ...
        <section className="card">
          ...
          <div className="rest-days">
            ...
            <div className="day-btns">
              {WEEKDAYS.map((name, i) => (
                <button key={i} className={"day-btn"+(restDays.includes(i)?" active":"")}
            ...
            {sortedMountains.map(m => (
              <div key={m.id} className="mountain-card" style={{ borderLeft: `4px solid ${m.color}` }}>
                <div className="mountain-info">
                  ...
                  <span className="mountain-date">
                    {new Date(m.date + "T12:00:00").toLocaleDateString("da-DK", { day:"numeric", month:"long", year:"numeric" })}
                  </span>
        ...
        <section className="card">
          <div className="tabs">
            ...
            <button className={"tab"+(activeTab==="log"  ?" active":"")} onClick={()=>setActiveTab("log")}>
              Loggede træninger <span className="log-count">{Object.keys(logged).length}</span>
            </button>
          </div>
              ...
              <div style={{ height: 320 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={series} margin={{ top: 8, right: 16, bottom: 0, left: -8 }}>
                    ...
                    {sortedMountains.map(m => {
                      const row = series.find(s => s.date === m.date);
                      return row ? (
                        <ReferenceLine key={m.id} x={row.label} stroke={m.color} strokeDasharray="4 2"
                          label={{ value:"🏔️ "+m.name, fill:m.color, fontSize:10, position:"top" }} />
                      ) : null;
                    })}
          ...
          {activeTab === "plan" && (
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>Dato</th><th>Session</th><th>Plan TSS</th><th>Logget TSS</th><th>CTL</th><th>ATL</th><th>TSB</th><th></th></tr>
                </thead>
                <tbody>
                  {series.map((row, i) => (
                    <tr key={i} className={row.isMountain?"row-mountain":row.tss===0?"row-rest":row.logged?"row-logged":""}>
                      ...
                      <td>{row.isMountain
                        ? <span style={{ color: row.mountainColor }}>🏔️ {row.mountainName}</span>
                        : row.planLabel}</td>
                      <td>{row.planTss||"—"}</td>
                      <td className="td-logged">
                        {row.logged
                          ? <span className="logged-badge">{row.tss} ✓{row.logNote ? ` · ${row.logNote.slice(0,20)}` : ""}</span>
                          : <span className="not-logged">—</span>}
                      </td>
                      <td>{row.ctl}</td><td>{row.atl}</td>
                      <td className={"td-tsb "+(row.tsb>5?"pos":row.tsb<-5?"neg":"")}>
                        {row.tsb>=0?"+":""}{row.tsb}
                      </td>
                      <td>
                        <button className="btn-log-row" onClick={() => setLogModal(row)}>
                          {row.logged?"✏️":"+"}
                        </button>
                      </td>
                    </tr>
    ...
    <label className="field">
      ...
      <input type="text" inputMode="decimal" value={displayVal}
        ...
        onBlur={() => {
          ...
          if (!isNaN(n)) { onChange(n); setRaw(String(n)); } else setRaw(String(value));
        }}
...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tt-date { color: #94a3b8; margin-bottom: 4px; font-weight: 600; }
  .tt-mountain { margin-bottom: 4px; }
  ...
  .import-source-title { font-size: 14px; font-weight: 700; color: #e2e8f0; margin-bottom: 8px; }
  .import-source-steps { display: flex; flex-direction: column; gap: 4px; }
  .step { font-size: 12px; color: #94a3b8; padding-left: 4px; }
  .step strong { color: #cbd5e1; }
